 ...
 *  Ngb_MaxPart + All.MaxNgb_Nodes....                reference "pseudo particles", i.e. the marker that indicates a top-node lying on another CPU.
 *
...
#if (NUM_THREADS > 1)
  omp_lock_t *node_lock = mymalloc("node_lock", NTopleaves * sizeof(omp_lock_t));
...
#if (NUM_THREADS > 1)
  Ngb_NodeLocks = mymalloc("Ngb_NodeLocks", Ngb_MaxNodes * sizeof(omp_lock_t));
  int i;
  for(i = 0; i < Ngb_MaxNodes; i++)
...
/*! This function frees the memory allocated for the neighbor tree.
 */
