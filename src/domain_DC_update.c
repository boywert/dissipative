      ...
      if(recvTask < NTask)
        if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
          MPI_Sendrecv(&send_trans_data[Send_offset[recvTask]],
                       ...
                       TAG_DENS_B, &recv_trans_data[Recv_offset[recvTask]],
                       Recv_count[recvTask] * sizeof(struct aux_trans_data), MPI_BYTE, recvTask, TAG_DENS_B, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
      ...
      if(recvTask < NTask)
        if(Send_count[recvTask] > 0 || Recv_count[recvTask] > 0)
          MPI_Sendrecv(&recv_trans_data[Recv_offset[recvTask]],
                       ...
                       TAG_DENS_B, &send_trans_data[Send_offset[recvTask]],
                       Send_count[recvTask] * sizeof(struct aux_trans_data), MPI_BYTE, recvTask, TAG_DENS_B, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
