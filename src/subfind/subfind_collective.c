...
void subfind_process_group_collectively(int nsubgroups_cat)
{
  ...
  for(master = 0, subnr = 0; master < SubNTask; master++)
    {
      ncand = count_cand;
      MPI_Bcast(&ncand, sizeof(int), MPI_INT, master, SubComm);
  ...
  int *count_loc = mymalloc("count_loc", sizeof(int) * GroupCat[GrNr].Nsubs);
  int *count_max = mymalloc("count_max", sizeof(int) * GroupCat[GrNr].Nsubs);
  int *count_sum = mymalloc("count_sum", sizeof(int) * GroupCat[GrNr].Nsubs);

  for(int i = 0; i < GroupCat[GrNr].Nsubs; i++)
    count_loc[i] = 0;

  for(int i = 0; i < NumPart; i++)
    {
      int subnr = (P[i].OriginalSubNr - GroupCat[GrNr].FirstSub);
      if(subnr >= 0 && subnr < GroupCat[GrNr].Nsubs)  
	count_loc[subnr]++;
    }

  MPI_Allreduce(count_loc, count_max, GroupCat[GrNr].Nsubs, MPI_INT, MPI_MAX, SubComm);
  MPI_Allreduce(count_loc, count_sum, GroupCat[GrNr].Nsubs, MPI_INT, MPI_SUM, SubComm);

  ud = (struct unbind_data *) mymalloc_movable(&ud, "ud", NumPartGroup * sizeof(struct unbind_data));

  subfind_loctree_treeallocate(All.TreeAllocFactor * NumPart, NumPart);

  Group[0].Nsubs = GroupCat[GrNr].Nsubs;

  for(int subnr = 0; subnr < GroupCat[GrNr].Nsubs; subnr++)
    {
      LocalLen = 0;
      int LocalNonGasLen;
      ...
      for(int i = 0; i < NumPart; i++)
        {
          if(P[i].OriginalSubNr == GroupCat[GrNr].FirstSub + subnr)
            ud[LocalLen++].index = i;
        }
