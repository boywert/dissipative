...
/* Same as generic_comm_pattern but you can pass the indices of the particles to be processed
 */
...
/* can replace
 * MPI_Alltoall(Send, sizeof(struct send_recv_counts), MPI_INT, Recv, sizeof(struct send_recv_counts), MPI_INT, MYCOMMUNICATOR);
 * with a space communication pattern that involves a non-blocking barrier (requires MPI-3.0)
 */
