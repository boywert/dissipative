...
/** Computes the total hydrodynamic cost of a particle i.
 *  If a cell is active on a certain timebin, it is assigned a cost of "1".
 *  All active timebins are summed, and the frequency with which each timebin is executed
...
#ifdef GFM
  if(P[i].Type == 4 || P[i].Type == 5)  /* for stars and BHs, count them like additional hydro cells because of their enrichment/accretion calculations */
...
void domain_combine_multipledomains(void)
{
  ...
  // MPI_Allreduce(loc_bin_Cost + lowestOccupiedTimeBin * ndomains, glob_bin_Cost + lowestOccupiedTimeBin * ndomains, 2 * ndomains * (All.HighestOccupiedTimeBin - loc_lowestOccupiedTimeBin + 1), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  ...
  for(int n = 0; n < ndomains; n++)
    {
      domainAssign[n].normalized_load = domainAssign[n].work / (tot_work + MIN_FLOAT_NUMBER) + domainAssign[n].worksph / (tot_worksph + MIN_FLOAT_NUMBER) + domainAssign[n].load / ((double) tot_load + MIN_FLOAT_NUMBER);
    }
