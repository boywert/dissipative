	  ...
	  for(idx = 0; idx < TimeBinsGravity.NActiveParticles; idx++)
	      ...
	      if(P[i].TimeBinGrav < 0)
		{
		  ...
		  fprintf(FdForceTest,
		      ...
		      P[i].PotentialDirect, P[i].PotentialShortRange, P[i].PotentialLongRange, P[i].Potential, P[i].PM_Potential);
#else
        ...
        if(nx != 0 || ny != 0)
          {
            ...
            double val = M_PI / (All.BoxSize * BOXX * All.BoxSize * BOXY) * sin(kx * x + ky * y) * (exp(k * z) * erfc(k / (2 * alpha) + alpha * z) +
                                                                                                    exp(-k * z) * erfc(k / (2 * alpha) - alpha * z)) / k;

            force[0] += -kx * val;
            force[1] += -ky * val;
            force[2] += M_PI / (All.BoxSize * BOXX * All.BoxSize * BOXY) * cos(kx * x + ky * y) * (exp(k * z) * erfc(k / (2 * alpha) + alpha * z) - exp(-k * z) * erfc(k / (2 * alpha) - alpha * z));
          }
      }
  ...
  for(int rep = 0; rep < 4 ; rep++)
    {
      force[rep] = Ewd_table[rep][i][j][k] * f1 +
          ...
          Ewd_table[rep][i + 1][j][k] * f5 +
          Ewd_table[rep][i + 1][j][k + 1] * f6 +
          Ewd_table[rep][i + 1][j + 1][k] * f7 +
          Ewd_table[rep][i + 1][j + 1][k + 1] * f8;
    }
