...
static void __attribute__ ((unused)) print_matrix(const int n, const double *matrix, const int *rowstart, const int *columns, const double *rhs, const int offset);

static void network_solver_integrate_internal(double temp, double rho, double *y, double maxtime, const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj);
static void network_solver_interpolate_traj_rhotemp(struct network_solver_trajectory *traj, double time, double *y, double *rho, double *temp);
static void network_solver_calc_dy(double temp, double rho, double dt, int steps, double *y, double *yn, const double *first_rhs,
                                   const jacob_t * old_jacob, const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj);
  ...
  for(i = 1; i < nsd->maxstep; i++)
    {
      for(k = 0; k < i; k++)
        {
          nsd->alf[i * nsd->maxstep + k] = pow(nsd->tolerance * 0.25, (nsd->a[k + 1] - nsd->a[i + 1]) / ((nsd->a[i + 1] - nsd->a[0] + 1.0) * (2. * (k + 1.0) + 1.0)));
        }
    }
                                   ...
                                   const jacob_t * old_jacob, const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj)
{
  ...
  dCreate_CompRow_Matrix(&A, nMatrix, nMatrix, jacob->n_elements, jacob->values, jacob->columns, jacob->rowstart, SLU_NR, SLU_D, SLU_GE);
                                   ...
                                   const jacob_t * old_jacob, const struct network_data *nd, struct network_workspace *nw, struct network_solver_trajectory *traj)
{
  ...
  phase = 12;
  PARDISO(pt, &maxfct, &mnum, &mtype, &phase, &matrixsize, values, rowstart, columns, &idum, &nrhs, iparm, &msglvl, &ddum, &ddum, &error);
  if(error != 0)
    {
  ...
  iparm[7] = refinement;        /* Max numbers of iterative refinement steps. */
  phase = 33;
  PARDISO(pt, &maxfct, &mnum, &mtype, &phase, &matrixsize, values, rowstart, columns, &idum, &nrhs, iparm, &msglvl, rhs, ddumarray, &error);
  if(error != 0)
    {
...
static void print_matrix(const int n, const double *matrix, const int *rowstart, const int *columns, const double *rhs, const int offset)
{
  int i, j;
  const int *col;
  const double *a;
