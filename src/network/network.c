...
static void compute_constant_screening_factors(struct network_rate *rate, const struct network_nucdata *nuc1, const struct network_nucdata *nuc2, const struct network_data *nd);
static network_var compute_screening(const struct network_rate *rate, network_var itemp13, network_var Gamma_prime, network_var lGamma_prime, network_var Gamma_prime14, network_var H_w_common);
...
static int network_init_internal(char *speciesfile, char *ratesfile, char *partfile, char *massesfile, char *weakratesfile, struct network_data *nd, int onlyweak);
  ...
  for(i = 0; i < nd->nuc_count; i++)
    {
      ...
      nd->nucdata[i].m = nd->nucdata[i].na * GSL_CONST_CGS_UNIFIED_ATOMIC_MASS + nd->nucdata[i].exm * conv / GSL_CONST_NUM_AVOGADRO / (GSL_CONST_CGS_SPEED_OF_LIGHT * GSL_CONST_CGS_SPEED_OF_LIGHT);
    }
  ...
  for(i = 0; i < nd->nuc_count; i++)
    {
      if(!spins[i])
        {
          myprintf("There are no partition function and spin data for nucleus %s. Assuming spin 0 and constant partition function of 1.\n", nd->nucdata[i].name);
        }
    }
  free(spins);
...
int network_getrhs(double rho, double temp, const double y[], int compute_derivs, const struct network_data *nd, struct network_workspace *nw, double *rhs, network_var * deriv)
{
  ...
  for(yrate = nw->yweakrates, weakratedata = nd->weakrates, rateval = nw->weakrates; weakratedata != weakrateend; yrate++, weakratedata++, rateval++)
    {
      /* The derivative w.r.t. y[i] is treated separately. */
  ...
  nucend = &nd->nucdata[nd->nuc_count];
  for(nucdata = nd->nucdata, rhsiter = deriv, pratelist = nw->prates, pweakratelist = nw->pweakrates; nucdata != nucend; nucdata++, rhsiter++, pratelist++, pweakratelist++)
    {
      network_var a = { 0 };
      network_var **prate;
      double *w, *wend;
      ...
      if(nd->weakrate_count > 0)
        {
          ...
          for(i = 0; i < nd->weakrate_count; i++)
            {
              b1 = n_ca(n_cm(dt, (nd->weakrates[i].lambda1[idx3] - nd->weakrates[i].lambda1[idx1]) / at), nd->weakrates[i].lambda1[idx1]);
              b2 = n_ca(n_cm(dt, (nd->weakrates[i].lambda1[idx4] - nd->weakrates[i].lambda1[idx2]) / at), nd->weakrates[i].lambda1[idx2]);
              rate1 = n_a(b1, n_cm(n_m(n_s(b2, b1), drhoye), 1.0 / arhoye));

              b1 = n_ca(n_cm(dt, (nd->weakrates[i].lambda2[idx3] - nd->weakrates[i].lambda2[idx1]) / at), nd->weakrates[i].lambda2[idx1]);
              b2 = n_ca(n_cm(dt, (nd->weakrates[i].lambda2[idx4] - nd->weakrates[i].lambda2[idx2]) / at), nd->weakrates[i].lambda2[idx2]);
...
static void compute_constant_screening_factors(struct network_rate *rate, const struct network_nucdata *nuc1, const struct network_nucdata *nuc2, const struct network_data *nd)
{
  ...
  rate->tau_fac =
    pow(27. / 2. * gsl_pow_2(M_PI) * (a1 * a2) / (a1 + a2) * gsl_pow_2(z1 * z2) * gsl_pow_4(ELECTRON_CHARGE_ESU) /
        (GSL_CONST_NUM_AVOGADRO * GSL_CONST_CGS_BOLTZMANN * gsl_pow_2(GSL_CONST_CGS_PLANCKS_CONSTANT_HBAR)), 1. / 3.);
   ...
   */
  rate->H_w_fac = gsl_pow_3(ELECTRON_CHARGE_ESU) * sqrt(4.0 * M_PI * GSL_CONST_NUM_AVOGADRO / gsl_pow_3(GSL_CONST_CGS_BOLTZMANN)) * z1 * z2;
  ...
  if(Gamma.v >= 0.3)
    {
      ...
      C = n_ca(n_a(n_a(n_cm(Gamma_prime, 0.896434 * rate->z_tilde1), n_cm(Gamma_prime14, -3.44740 * rate->z_tilde2)), n_cm(lGamma_prime, -0.5551)), rate->C_const);
      ...
      H_s = n_a(C,
                n_a(n_a(n_a(n_m(n_powi(b, 3),
                                n_cm(tau, -5. / (32. * 3.))),
                            n_m(n_powi(b, 4),
                                n_s(n_cm(tau, 0.014 / 3.), n_cm(Gamma, 0.0055)))), n_m(n_powi(b, 5), n_a(n_cm(tau, 0.0128 / 3.), n_cm(Gamma, 0.0098)))), n_m(n_powi(b, 6), n_cm(Gamma, -0.0048))));
    }
