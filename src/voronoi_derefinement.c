          ...
          if(deref_SphP[i].Flag == 2)
            {
              ...
              /*
                 double vol2 = 0;
                 for(k=0; k < n_tri; k++)
                 vol2 += get_tri_volume(k, trilist);

                 printf("task=%d: after split ntri=%d  Volume=%g  %g  %g  rel=%g\n", ThisTask,  n_tri, 
                 SphP[i].Volume, vol, vol2, vol2/SphP[i].Volume );
                 ...
                 {
                 vol = get_tri_volume(k, trilist);
                 printf("TASK=%d i=%d k=%d vol=%g  ratio=%g\n", ThisTask, i, k, vol, vol/vol2); 
                 }
                 terminate("reached too many");
                 }
                      ...
                      if(k == k_last)
                        {
                          if(fac > 0 && fabs(fac * P[i].Mass / CellTracerMass - 1) > 1.0e-3)
                            terminate("fac * P[i].Mass / CellTracermass=%g", fac * P[i].Mass / CellTracerMass);
...
static void derefine_apply_flux_list(void)
{
  ...
  for(i = 0; i < nimport; i++)
      ...
      for(int num1=0;num1<MRT_BINS;num1++)
	{
      ...
      int m;
      for(m = 0; m < TGCHEM_NUM_ABUNDANCES; m++)
        SphP[p].Abund[m] = (SphP[p].Abund[m] * SphP[p].Volume + FluxListGet[i].dAbund[m] * FluxListGet[i].dVolume) / (SphP[p].Volume + FluxListGet[i].dVolume);
#endif
#ifdef REFINEMENT_SPLIT_CELLS
      FlagDoNotRefine[p] = 1;
#endif
