...
//! calculate the cross section between a plane and a Voronoi cell (in 3D)
/*!
 *
 *  todo: test for numerical instabilities
 *
 *  \param sphp_index The hydro index of the cell
 *  \param dp_index The delaunay point index of the cell
 *  \param center A point in the plane
 *  \param n A vector starting at center and normal to the plane
 *  \result The cross section between the plane and the cell
 */
#ifdef TETRA_INDEX_IN_FACE
...
//! calculate the intersections bewtween a plane and a cell
/*!
 *
 *  \param sphp_index The hydro index of the cell
 *  \param dp_index The delaunay point index of the cell
 *  \param center A point in the plane
 *  \param n A vector starting at center and normal to the plane
 *  \param polygon Store the intersections (polygon) in this array (output)
 *  \param nof_polygon_elements The number of stored elements in the polygon array (output)
 */
...
//! write out the intersections between a plane and the grid (for plotting)
/*!
 *
 * Binary output:
 * int: number of elements in the first array
 * int: number of elements in the second array
 * int[]: array, which stores the number of intersections for each intersected cell.
 *        The j-th entry gives the number of elements in the intersections array which correspond to the j-th intersected cell.
 * double[]: intersections array, all intersections are stored in the order x1,y1,x2,y2,x3,y3,...
 *
 * The intersections are given in a coordinate system where n is the z-axis and which
 * has its origin at center.
 *
 *  \param center A point in the plane
 *  \param n A vector starting at center and normal to the plane
 */
 ...
 *
 *  \param polygon Array of points of the polygon: x1, y1, x2, y2, ..., has to be sorted counterclockwise and closed (x_n == x_0 && y_n == y_0)
 *  \param nof_elements Number of elements in the array
 *
 */
static double polygon_area(double *polygon, unsigned int nof_elements)
{
